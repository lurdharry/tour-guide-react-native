...
import { Animated, Dimensions, Easing, View, type LayoutChangeEvent } from 'react-native';
...
const defaultSvgPath: SvgMaskPathFunction = ({ size, position, canvasSize }): string => {
...
export const SvgMask = ({
  ...
  const sizeValue = useRef<Animated.ValueXY>(new Animated.ValueXY(size)).current;
  const positionValue = useRef<Animated.ValueXY>(new Animated.ValueXY(position)).current;
  ...
  return (
    <View style={style} onLayout={handleLayout} onStartShouldSetResponder={onClick}>
      {canvasSize ? (
